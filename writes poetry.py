...
def neuralnetwork(model='lstm', batchsize = batch_size,rnn_size=128, num_layers=3):
    if model == 'rnn':
    ...
    cell = tf.nn.rnn_cell.MultiRNNCell([get_a_cell() for _ in range(num_layers)])  # n层RNN
...
def train_neural_network():
    ...
    with tf.Session() as sess:
        ...
        if os.path.exists('save/checkpoint'):  # 判断模型是否存在
            saver.restore(sess, 'save/model.ckpt-100')  # 存在就从模型中恢复变量
        else:
            sess.run(tf.global_variables_initializer())


        for epoch in range(1001):
            sess.run(tf.assign(learning_rate, 0.0000001 * (0.05 ** epoch)))
            n = 0
...
def gen(char_num):
    ...
    with tf.Session() as sess:
        ...
        def getchar (char_num = len(words)):
            if functions.is_chinese(char_num):
                return char_num
            else:
                r = random.randint(1, int(char_num))
                char = get_keys(word_map, r)[0]
                return char

        def again (char):
            if functions.is_chinese(char):
                return False
            else:
                print('again')
                return True
        ...
        char = getchar(char_num)
        while again(char):
            char = getchar(char_num)

        x = np.array([list(map(word_map.get, char ))])
